import { Layout } from "@/components/Layout";
import { AntigravityBg } from "@/components/home/AntigravityBg";
import { Container } from "@/components/layout/Container";
import { Section } from "@/components/layout/Section";
import { Sparkles, Mail, Phone } from "lucide-react";

export default function Support() {
  return (
    <Layout>
      <AntigravityBg />
      
      <Section className="relative z-10 pt-32 pb-20">
        <Container>
          <div className="max-w-4xl mx-auto text-center">
            <div className="animate-slide-up" style={{ animationDelay: '100ms', animationFillMode: 'both' }}>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/70 border border-gray-200/50 mb-8 backdrop-blur-xl shadow-sm">
                <Sparkles className="w-4 h-4 text-gray-900" />
                <span className="text-sm font-semibold tracking-tight text-gray-800">Help & Support</span>
              </div>
            </div>
            
            <h1 className="animate-slide-up text-5xl md:text-6xl font-bold text-gray-900 leading-[1.05] tracking-tight mb-6" style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
              We're Here to Help
            </h1>
            
            <p className="animate-slide-up text-xl font-medium text-gray-500 max-w-2xl mx-auto leading-snug tracking-tight" style={{ animationDelay: '300ms', animationFillMode: 'both' }}>
              Get support from our team. We respond quickly to help you succeed.
            </p>
          </div>
        </Container>
      </Section>

      <Section className="py-24 relative z-10 border-t border-gray-200/50 bg-gray-50/60">
        <Container>
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div className="bg-white/80 backdrop-blur-sm p-8 rounded-[24px] border border-gray-200/60 shadow-sm">
              <Mail className="w-8 h-8 text-gray-900 mb-4" />
              <h3 className="text-lg font-bold text-gray-900 mb-2">Email Support</h3>
              <a href="mailto:support@dcoresystems.com" className="text-gray-900 hover:underline">support@dcoresystems.com</a>
            </div>
            <div className="bg-white/80 backdrop-blur-sm p-8 rounded-[24px] border border-gray-200/60 shadow-sm">
              <Phone className="w-8 h-8 text-gray-900 mb-4" />
              <h3 className="text-lg font-bold text-gray-900 mb-2">WhatsApp Support</h3>
              <a href="tel:+919766521300" className="text-gray-900 hover:underline">+91 97665 21300</a>
            </div>
          </div>
        </Container>
      </Section>
    </Layout>
  );
}

